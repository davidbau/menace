...
for (let i = 0; i < assignments.length; i++) {
    ...
    const attackFleeLoopBreakTurns = extractInt(text, /Attack decision telemetry:\s+fleeLoopBreak=(\d+)/);
    const attackForcedCorneredTurns = extractInt(text, /Attack decision telemetry:.*\bforced=(\d+)/);
    const attackBlockingTurns = extractInt(text, /Attack decision telemetry:.*\bblocking=(\d+)/);
    const attackLoneDogTurns = extractInt(text, /Attack decision telemetry:.*\bloneDog=(\d+)/);
    const attackOtherTurns = extractInt(text, /Attack decision telemetry:.*\bother=(\d+)/);
    ...
    const doorKick = extractInt(text, /Explore telemetry:.*\bdoorKick=(\d+)/);
        ...
        const row = {
            ...
            attackFleeLoopBreakTurns: Number.isFinite(attackFleeLoopBreakTurns) ? attackFleeLoopBreakTurns : null,
            attackForcedCorneredTurns: Number.isFinite(attackForcedCorneredTurns) ? attackForcedCorneredTurns : null,
            attackBlockingTurns: Number.isFinite(attackBlockingTurns) ? attackBlockingTurns : null,
            attackLoneDogTurns: Number.isFinite(attackLoneDogTurns) ? attackLoneDogTurns : null,
            attackOtherTurns: Number.isFinite(attackOtherTurns) ? attackOtherTurns : null,
...
const avgAttackFleeLoopBreakTurns = avgOf(results.map(r => r.attackFleeLoopBreakTurns));
const avgAttackForcedCorneredTurns = avgOf(results.map(r => r.attackForcedCorneredTurns));
const avgAttackBlockingTurns = avgOf(results.map(r => r.attackBlockingTurns));
const avgAttackLoneDogTurns = avgOf(results.map(r => r.attackLoneDogTurns));
const avgAttackOtherTurns = avgOf(results.map(r => r.attackOtherTurns));
...
console.log(`  Attack decision avg: fleeLoopBreak=${fmtAvg(avgAttackFleeLoopBreakTurns)} forced=${fmtAvg(avgAttackForcedCorneredTurns)} blocking=${fmtAvg(avgAttackBlockingTurns)} loneDog=${fmtAvg(avgAttackLoneDogTurns)} other=${fmtAvg(avgAttackOtherTurns)}`);
...
const summary = {
    ...
    avgAttackTurns,
    avgFleeTurns,
    avgAttackFleeLoopBreakTurns,
    avgAttackForcedCorneredTurns,
    avgAttackBlockingTurns,
    avgAttackLoneDogTurns,
    avgAttackOtherTurns,
