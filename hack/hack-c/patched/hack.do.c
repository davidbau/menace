...
register struct obj *otmp;
{
	...
	case 13:
		pline("You found a map!");
		{
			...
			for(foo=(struct rm *)levl;foo<(struct rm *)&levl[79][21];foo++) {
				if(foo->typ==SDOOR) {
					foo->typ=DOOR;
					foo->scrsym='+';
					foo->new=1;
					on((foo-(struct rm *)levl)%80,(foo-(struct rm *)levl)/80);
				} else if((foo->typ==CORR || foo->typ==WALL ||
foo->typ==DOOR) && !foo->seen) foo->new=1,on((foo-(struct rm *)levl)%80,(foo-(struct rm *)levl)/80);
			}
		}
