#!/usr/bin/env node
// Crop colored ASCII art HTML to 80 columns, no scaling.
// Just takes a horizontal window and optionally trims rows.
...
const file = process.argv[2] || '/Users/davidbau/Downloads/colored-ascii-art (3).html';
...
// Crop: center 80 cols, take first 13 rows (or all if <= 13)
const tgtW = 80;
...
for (let ty = 0; ty < tgtH; ty++) {
    ...
    for (let tx = 0; tx < tgtW; tx++) {
        ...
        if (!cell || cell.ch === ' ') {
            const ci = cell ? nearestColor(cell.r, cell.g, cell.b) : -1;
            chars += ' ';
            colors.push(ci);
            continue;
        }
