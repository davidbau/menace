...
|------|----------|---------|
| `const.js` | All hand-maintained literals: scalar constants, display tables, direction arrays, terrain/symbol/trap constants | `version.js` only (for `COMMIT_NUMBER`) |
...
`const.js` and `const.js` are merged into `const.js`. `const.js`
currently imports level-type constants from `const.js`; merging eliminates
...
1. Consolidate all hand-maintained constants in `const.js`
