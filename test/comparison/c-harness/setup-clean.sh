...
PATCHES_DIR="$SCRIPT_DIR/patches-clean"
...
DEFERRED_PATCH="008-keylog-input-tracing.patch"
...
if [[ -f "$PATCHES_DIR/$DEFERRED_PATCH" ]]; then
    # 008 depends on earlier harness symbols (cmd.c/wintty.c support), so it is
...
require_marker "src/mklev.c" 'midlog_enter("join", __FILE__, __LINE__, __func__)' "midlog join trace"
