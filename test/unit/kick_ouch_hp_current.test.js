...
test('kick_ouch subtracts from current HP (uhp), not max HP (uhpmax)', async () => {
    ...
    const nearSleeper = { mx: 10, my: 15, dead: false, mhp: 5, msleeping: 1, mstrategy: 0 };
    const farSleeper = { mx: 20, my: 20, dead: false, mhp: 5, msleeping: 1, mstrategy: 0 };
    const map = {
        rooms: [],
        engravings: [],
    ...
    assert.ok(farSleeper.msleeping);
});
