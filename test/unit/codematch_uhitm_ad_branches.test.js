...
test('mhitm_ad_were delegates through physical handling (kick vs thick hide -> zero damage)', async () => {
...
test('mhitm_ad_famn zeroes damage only for non-eaters', async () => {
    const magr = { mcan: false, data: {} };
    const mattk = { adtyp: 0 };

    const nonEater = { data: { mflags1: 0 } };
    const mhm1 = { damage: 6 };
    await mhitm_ad_famn(magr, mattk, nonEater, mhm1);
    assert.equal(mhm1.damage, 0);

    const eater = { data: { mflags1: M1_HERBIVORE } };
    const mhm2 = { damage: 6 };
    await mhitm_ad_famn(magr, mattk, eater, mhm2);
...
test('mhitm_ad_curs can cancel defender and preserve base damage', async () => {
    initRng(9);
    ...
    for (let i = 0; i < 200; i++) {
        ...
        await mhitm_ad_curs(magr, {}, mdef, mhm);
        if (mdef.mcan === 1) {
...
test('mhitm_ad_curs destroys clay golem target on curse branch', async () => {
    initRng(13);
    const magr = { mcan: false, data: { mlet: 'd' } };
    let observed = false;
    for (let i = 0; i < 300; i++) {
...
test('mhitm_ad_sgld transfers defender gold to attacker inventory', async () => {
    ...
    await mhitm_ad_sgld(magr, {}, mdef, mhm);
    assert.equal(mhm.damage, 0);
...
test('mhitm_ad_sgld steals from same-class target in m-vs-m branch', async () => {
    const magr = { mcan: false, data: { mlet: 'n' }, minvent: [] };
...
test('mhitm_ad_dgst swallows non-rider defender (damage becomes defender hp)', async () => {
    const mhm = { damage: 1, hitflags: 0, done: false };
    const mdef = { mhp: 23, data: { mlet: 'o' } };
    await mhitm_ad_dgst({ mhp: 20, data: {} }, {}, mdef, mhm);
    assert.equal(mhm.damage, 23);
    assert.equal(mhm.done, false);
});

test('mhitm_ad_dgst marks aggressor death when digesting a Rider', async () => {
    ...
    await mhitm_ad_dgst(magr, {}, mdef, mhm);
    assert.equal(mhm.done, true);
