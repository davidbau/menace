import { describe, it, beforeEach } from 'node:test';
import assert from 'node:assert/strict';

import {
    createInputQueue,
    setInputRuntime,
    getInputRuntime,
    pushInput,
    nhgetch,
    clearInputQueue,
} from '../../js/input.js';
import { mapBrowserKeyToNhCode } from '../../js/browser_input.js';

describe('input runtime primitives', () => {
    beforeEach(() => {
        setInputRuntime(createInputQueue());
    });

    it('createInputQueue yields pushed input in order', async () => {
        const runtime = createInputQueue();
        runtime.pushInput('a'.charCodeAt(0));
        runtime.pushInput('b'.charCodeAt(0));
        assert.equal(await runtime.nhgetch(), 'a'.charCodeAt(0));
        assert.equal(await runtime.nhgetch(), 'b'.charCodeAt(0));
    });

    it('module wrappers use active input runtime', async () => {
        const runtime = createInputQueue();
        setInputRuntime(runtime);
        pushInput('x'.charCodeAt(0));
        assert.equal(await nhgetch(), 'x'.charCodeAt(0));
        assert.equal(getInputRuntime(), runtime);
    });

    it('clearInputQueue clears queued input on active runtime', async () => {
        const runtime = createInputQueue();
        setInputRuntime(runtime);
        pushInput('x'.charCodeAt(0));
        clearInputQueue();

        let settled = false;
        const p = nhgetch().then(() => {
            settled = true;
        });
        await new Promise((r) => setTimeout(r, 10));
        assert.equal(settled, false);

        pushInput('y'.charCodeAt(0));
        await p;
    });

    it('createInputQueue hasInput returns correct state', () => {
        const runtime = createInputQueue();
        assert.equal(runtime.hasInput(), false);
        runtime.pushInput('a'.charCodeAt(0));
        assert.equal(runtime.hasInput(), true);
    });
});

describe('browser key mapping', () => {
    it('maps numpad keys when number_pad is enabled', () => {
        const code = mapBrowserKeyToNhCode(
            { key: '8', location: 3, ctrlKey: false, altKey: false, metaKey: false },
            { number_pad: true }
        );
        assert.equal(code, 'k'.charCodeAt(0));
    });

    it('ignores numpad keys when number_pad is disabled', () => {
        const code = mapBrowserKeyToNhCode(
            { key: '8', location: 3, ctrlKey: false, altKey: false, metaKey: false },
            { number_pad: false }
        );
        assert.equal(code, '8'.charCodeAt(0));
    });

    it('maps space to rest only when rest_on_space is enabled', () => {
        const enabled = mapBrowserKeyToNhCode(
            { key: ' ', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            { rest_on_space: true }
        );
        const disabled = mapBrowserKeyToNhCode(
            { key: ' ', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            { rest_on_space: false }
        );
        assert.equal(enabled, '.'.charCodeAt(0));
        assert.equal(disabled, ' '.charCodeAt(0));
    });

    it('maps arrow keys to vi movement', () => {
        const left = mapBrowserKeyToNhCode(
            { key: 'ArrowLeft', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            {}
        );
        const right = mapBrowserKeyToNhCode(
            { key: 'ArrowRight', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            {}
        );
        const up = mapBrowserKeyToNhCode(
            { key: 'ArrowUp', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            {}
        );
        const down = mapBrowserKeyToNhCode(
            { key: 'ArrowDown', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            {}
        );
        assert.equal(left, 'h'.charCodeAt(0));
        assert.equal(right, 'l'.charCodeAt(0));
        assert.equal(up, 'k'.charCodeAt(0));
        assert.equal(down, 'j'.charCodeAt(0));
    });

    it('maps ctrl-letter combinations to C() codes', () => {
        const ctrlA = mapBrowserKeyToNhCode(
            { key: 'a', location: 0, ctrlKey: true, altKey: false, metaKey: false },
            {}
        );
        const ctrlZ = mapBrowserKeyToNhCode(
            { key: 'z', location: 0, ctrlKey: true, altKey: false, metaKey: false },
            {}
        );
        assert.equal(ctrlA, 1);  // Ctrl+A = 1
        assert.equal(ctrlZ, 26); // Ctrl+Z = 26
    });

    it('maps alt-letter combinations to M() codes', () => {
        const altA = mapBrowserKeyToNhCode(
            { key: 'a', location: 0, ctrlKey: false, altKey: true, metaKey: false },
            {}
        );
        assert.equal(altA, 'a'.charCodeAt(0) | 0x80);
    });

    it('maps escape key', () => {
        const code = mapBrowserKeyToNhCode(
            { key: 'Escape', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            {}
        );
        assert.equal(code, 27);
    });

    it('maps enter key', () => {
        const code = mapBrowserKeyToNhCode(
            { key: 'Enter', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            {}
        );
        assert.equal(code, 13);
    });

    it('maps backspace key', () => {
        const code = mapBrowserKeyToNhCode(
            { key: 'Backspace', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            {}
        );
        assert.equal(code, 8);
    });

    it('ignores modifier-only keys', () => {
        const shift = mapBrowserKeyToNhCode(
            { key: 'Shift', location: 0, ctrlKey: false, altKey: false, metaKey: false },
            {}
        );
        const ctrl = mapBrowserKeyToNhCode(
            { key: 'Control', location: 0, ctrlKey: true, altKey: false, metaKey: false },
            {}
        );
        assert.equal(shift, null);
        assert.equal(ctrl, null);
    });
});
