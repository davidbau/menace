...
PIT, SPIKED_PIT, SQKY_BOARD, TELEP_TRAP, LEVEL_TELEP, MAGIC_PORTAL, ROLLING_BOULDER_TRAP,
POLY_NOFLAGS, POLY_CONTROLLED, POLY_MONSTER, POLY_REVERT, POLY_LOW_CTRL } from './const.js';
...
async function polyman(player, fmt, arg) {
    ...
    if (Upolyd) {
        ...
        if (player.macurr && player.macurr.str !== undefined) {
            player.attributes[A_STR] = player.macurr.str;
        }
    }
    set_uasmon(player);
...
export async function polymon(player, mntmp, map) {
    ...
    if (strongmonst(mons[mntmp])) {
        ...
        player.attributes[A_STR] = newMaxStr;
    } else {
        if (player.amax) player.amax.str = newMaxStr;
        if (player.acurr && player.acurr.str > newMaxStr)
            player.acurr.str = newMaxStr;
        player.attributes[A_STR] = Math.min(player.attributes[A_STR] || newMaxStr, newMaxStr);
    }
...
export async function drop_weapon(player, alone) {
    ...
    if (player.weapon) {
        if (!alone || cantwield(player.type)) {
            if (alone) {
                const wname = xname(player.weapon);
                await You("find you must drop your %s!", wname);
            }
