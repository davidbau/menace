...
export function generate() {
    // NetHack core nhcore.lua	$NHDT-Date: 1652196284 2022/5/10 15:24:44 $  $NHDT-Branch: NetHack-3.7 $:$NHDT-Revision: 1.0 $
    // Copyright (c) 2021 by Pasi Kallinen
    // NetHack may be freely redistributed.  See license for details.
    // This file contains lua code used by NetHack core.
    // Is it loaded once, at game start, && kept in memory until game exit.

    // Data in nh_lua_variables table can be set && queried with nh.variable()
    // This table is saved && restored along with the game.
    nh_lua_variables: [
    ];
    ...
    function mk_dgl_extrainfo() {
        if (((prev_dgl_extrainfo == 0) || (prev_dgl_extrainfo + 50 < u.moves))) {
            ...
            let extrai, err: io.open(filename, "w");
            if (extrai) {
                let sortval = 0;
                let dname = nh.dnum_name(u.dnum);
                let dstr = "";
                let astr = " ";
                if (u.uhave_amulet == 1) {
