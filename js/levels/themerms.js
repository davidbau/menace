...
export function generate() {
    ...
    themeroom_fills = {  { name = "Ice room", contents = function(rm) ice = selection.room(); des.terrain(ice, "I"); if ((percent(25))) { mintime = 1000 - (nh.level_difficulty() * 100); ice_melter = function(x,y) nh.start_timer_at(x,y, "melt-ice", mintime + nh.rn2(1000)); } ice.iterate(ice_melter); } end, },  { name = "Cloud room", contents = function(rm) fog = selection.room(); for i = 1, (fog.numpoints() / 4) do des.monster({ id = "fog cloud", asleep = true }); } des.gas_cloud({ selection = fog }); end, },  { name = "Boulder room", mindiff = 4, contents = function(rm) locs = selection.room():percentage(30); func = function(x,y) if ((percent(50))) { des.object("boulder", x, y); else des.trap("rolling boulder", x, y); } } locs.iterate(func); end, },  { name = "Spider nest", contents = function(rm) spooders = nh.level_difficulty() > 8; locs = selection.room():percentage(30); func = function(x,y) des.trap({ type = "web", x = x, y = y, spider_on_web = spooders && percent(80) }); } locs.iterate(func); end, },  { name = "Trap room", contents = function(rm) traps = { "arrow", "dart", "falling rock", "bear", "land mine", "sleep gas", "rust", "anti magic" }; shuffle(traps); locs = selection.room():percentage(30); func = function(x,y) des.trap(traps[1], x, y); } locs.iterate(func); end, },  { name = "Garden", eligible = function(rm) return rm.lit === true; end, contents = function(rm) s = selection.room(); npts = (s.numpoints() / 6); for i = 1, npts do des.monster({ id = "wood nymph", asleep = true }); if ((percent(30))) { des.feature("fountain"); } } table.insert(postprocess, { handler = make_garden_walls, data = { sel = selection.room() } }); } },  { name = "Buried treasure", contents = function(rm) des.object({ id = "chest", buried = true, contents = function(otmp) xobj = otmp.totable(); //  keep track of the last buried treasure if ((xobj.NO_OBJ == nil)) { table.insert(postprocess, { handler = make_dig_engraving, data = { x = xobj.ox, y = xobj.oy }}); } for i = 1, d(3,4) do des.object(); } } }); end, },  { name = "Buried zombies", contents = function(rm) local diff = nh.level_difficulty() -- start with [1..4] for low difficulty local zombifiable = { "kobold", "gnome", "orc", "dwarf" }; if (diff > 3) {          -- medium difficulty zombifiable[5], zombifiable[6] = "elf", "human"; if (diff > 6) {       -- high difficulty (relatively speaking) zombifiable[7], zombifiable[8] = "ettin", "giant"; } } for i = 1, (rm.width * rm.height) / 2 do shuffle(zombifiable); local o = des.object({ id = "corpse", montype = zombifiable[1], buried = true }); o:stop_timer("rot-corpse"); o:start_timer("zombify-mon", math.random(990, 1010)); } end, },  { name = "Massacre", contents = function(rm) local mon = { "apprentice", "warrior", "ninja", "thug", "hunter", "acolyte", "abbot", "page", "attendant", "neanderthal", "chieftain", "student", "wizard", "valkyrie", "tourist", "samurai", "rogue", "ranger", "priestess", "priest", "monk", "knight", "healer", "cavewoman", "caveman", "barbarian", "archeologist" }; local idx = math.random(#mon); for i = 1, d(5,5) do if ((percent(10))) { idx = math.random(#mon); } des.object({ id = "corpse", montype = mon[idx] }); } end, },  { name = "Statuary", contents = function(rm) for i = 1, d(5,5) do des.object({ id = "statue" }); } for i = 1, d(3) do des.trap("statue"); } end, },   { name = "Light source", eligible = function(rm) return rm.lit == false; end, contents = function(rm) des.object({ id = "oil lamp", lit = true }); } },  { name = "Temple of the gods", contents = function(rm) des.altar({ align = align[1] }); des.altar({ align = align[2] }); des.altar({ align = align[3] }); end, },  { name = "Ghost of an Adventurer", contents = function(rm) local loc = selection.room():rndcoord(0); des.monster({ id = "ghost", asleep = true, waiting = true, coord = loc }); if (percent(65)) { des.object({ id = "dagger", coord = loc, buc = "not-blessed" }); } if (percent(55)) { des.object({ class = ")", coord = loc, buc = "not-blessed" }); } if (percent(45)) { des.object({ id = "bow", coord = loc, buc = "not-blessed" }); des.object({ id = "arrow", coord = loc, buc = "not-blessed" }); } if (percent(65)) { des.object({ class = "[", coord = loc, buc = "not-blessed" }); } if (percent(20)) { des.object({ class = "=", coord = loc, buc = "not-blessed" }); } if (percent(20)) { des.object({ class = "?", coord = loc, buc = "not-blessed" }); } end, },  { name = "Storeroom", contents = function(rm) local locs = selection.room():percentage(30); local func = function(x,y) if ((percent(25))) { des.object("chest"); else des.monster({ class = "m", appear_as = "obj:chest" }); } } locs:iterate(func); end, },  { name = "Teleportation hub", contents = function(rm) local locs = selection.room():filter_mapchar("."); for i = 1, 2 + nh.rn2(3) do local pos = locs:rndcoord(1); if ((pos.x > 0)) { pos.x = pos.x + rm.region.x1 - 1; pos.y = pos.y + rm.region.y1; table.insert(postprocess, { handler = make_a_trap, data = { type = "teleport", seen = true, coord = pos, teledest = 1 } }); } } end, }, };

    themerooms = { { name = "default", frequency = 1000, contents = function() des.room({ type = "ordinary", filled = 1 }); } },  { name = "Fake Delphi", contents = function() des.room({ type = "ordinary", w = 11,h = 9, filled = 1, contents = function() des.room({ type = "ordinary", x = 4,y = 3, w = 3,h = 3, filled = 1, contents = function() des.door({ state="random", wall="all" }); } }); } }); end, },  { name = "Room in a room", contents = function() des.room({ type = "ordinary", filled = 1, contents = function() des.room({ type = "ordinary", contents = function() des.door({ state="random", wall="all" }); } }); } }); end, },  { name = "Huge room with another room inside", contents = function() des.room({ type = "ordinary", w = nh.rn2(10)+11,h = nh.rn2(5)+8, filled = 1, contents = function() if ((percent(90))) { des.room({ type = "ordinary", filled = 1, contents = function() des.door({ state="random", wall="all" }); if ((percent(50))) { des.door({ state="random", wall="all" }); } } }); } } }); end, },  { name = "Nesting rooms", contents = function() des.room({ type = "ordinary", w = 9 + nh.rn2(4), h = 9 + nh.rn2(4), filled = 1, contents = function(rm) wid = Math.random(Math.floor(rm.width / 2), rm.width - 2); hei = Math.random(Math.floor(rm.height / 2), rm.height - 2); des.room({ type = "ordinary", w = wid,h = hei, filled = 1, contents = function() if ((percent(90))) { des.room({ type = "ordinary", filled = 1, contents = function() des.door({ state="random", wall="all" }); if ((percent(15))) { des.door({ state="random", wall="all" }); } } }); } des.door({ state="random", wall="all" }); if ((percent(15))) { des.door({ state="random", wall="all" }); } } }); } }); end, },  { name = "Default room with themed fill", frequency = 6, contents = function() des.room({ type = "themed", contents = themeroom_fill }); } },  { name = "Unlit room with themed fill", frequency = 2, contents = function() des.room({ type = "themed", lit = 0, contents = themeroom_fill }); } },  { name = "Room with both normal contents && themed fill", frequency = 2, contents = function() des.room({ type = "themed", filled = 1, contents = themeroom_fill }); } },  { name = 'Pillars', contents = function() des.room({ type = "themed", w = 10, h = 10, contents = function(rm) terr = { "-", "-", "-", "-", "L", "P", "T" }; shuffle(terr); for x = 0, (rm.width / 4) - 1 do for y = 0, (rm.height / 4) - 1 do des.terrain({ x = x * 4 + 2, y = y * 4 + 2, typ = terr[1], lit = -2 }); des.terrain({ x = x * 4 + 3, y = y * 4 + 2, typ = terr[1], lit = -2 }); des.terrain({ x = x * 4 + 2, y = y * 4 + 3, typ = terr[1], lit = -2 }); des.terrain({ x = x * 4 + 3, y = y * 4 + 3, typ = terr[1], lit = -2 }); } } } }); end, },  { name = 'Mausoleum', contents = function() des.room({ type = "themed", w = 5 + nh.rn2(3)*2, h = 5 + nh.rn2(3)*2, contents = function(rm) des.room({ type = "themed", x = (rm.width - 1) / 2, y = (rm.height - 1) / 2, w = 1, h = 1, joined = false, contents = function() if ((percent(50))) { mons = { "M", "V", "L", "Z" }; shuffle(mons); des.monster({ class = mons[1], x=0,y=0, waiting = 1 }); else des.object({ id = "corpse", montype = "@", coord = {0,0} }); } if ((percent(20))) { des.door({ state="secret", wall="all" }); } } }); } }); end, },  { name = 'Random dungeon feature in the middle of an odd-sized room', contents = function() wid = 3 + (nh.rn2(3) * 2); hei = 3 + (nh.rn2(3) * 2); des.room({ type = "ordinary", filled = 1, w = wid, h = hei, contents = function(rm) feature = { "C", "L", "I", "P", "T" }; shuffle(feature); des.terrain((rm.width - 1) / 2, (rm.height - 1) / 2, feature[1]); } }); end, },  { name = 'L-shaped', contents = function() des.map({ map = [[ // ---xxx |...|xxx |...|xxx |...---- |......| |......| |......| --------]], contents = function(m) filler_region(1,1); } }); end, },  { name = 'L-shaped, rot 1', contents = function() des.map({ map = [[ xxx----- xxx|...| xxx|...| ----...| |......| |......| |......| --------]], contents = function(m) filler_region(5,1); } }); end, },  { name = 'L-shaped, rot 2', contents = function() des.map({ map = [[ -------- |......| |......| |......| ----...| xxx|...| xxx|...| xxx-----]], contents = function(m) filler_region(1,1); } }); end, },  { name = 'L-shaped, rot 3', contents = function() des.map({ map = [[ -------- |......| |......| |......| |...---- |...|xxx |...|xxx -----xxx]], contents = function(m) filler_region(1,1); } }); end, },  { name = 'Blocked center', contents = function() des.map({ map = [[ ----------- |.........| |.........| |.........| |...LLL...| |...LLL...| |...LLL...| |.........| |.........| |.........| -----------]], contents = function(m) if ((percent(30))) { local terr = { "-", "P" }; shuffle(terr); des.replace_terrain({ region = {1,1, 9,9}, fromterrain = "L", toterrain = terr[1] }); } filler_region(1,1); } }); end, },  { name = 'Circular, small', contents = function() des.map({ map = [[ xx---xx x--.--x --...-- |.....| --...-- x--.--x xx---xx]], contents = function(m) filler_region(3,3); } }); end, },  { name = 'Circular, medium', contents = function() des.map({ map = [[ xx-----xx x--...--x --.....-- |.......| |.......| |.......| --.....-- x--...--x xx-----xx]], contents = function(m) filler_region(4,4); } }); end, },  { name = 'Circular, big', contents = function() des.map({ map = [[ xxx-----xxx x---...---x x-.......-x --.......-- |.........| |.........| |.........| --.......-- x-.......-x x---...---x xxx-----xxx]], contents = function(m) filler_region(5,5); } }); end, },  { name = 'T-shaped', contents = function() des.map({ map = [[ xxx-----xxx xxx|...|xxx xxx|...|xxx ----...---- |.........| |.........| |.........| -----------]], contents = function(m) filler_region(5,5); } }); end, },  { name = 'T-shaped, rot 1', contents = function() des.map({ map = [[ -----xxx |...|xxx |...|xxx |...---- |......| |......| |......| |...---- |...|xxx |...|xxx -----xxx]], contents = function(m) filler_region(2,2); } }); end, },  { name = 'T-shaped, rot 2', contents = function() des.map({ map = [[ ----------- |.........| |.........| |.........| ----...---- xxx|...|xxx xxx|...|xxx xxx-----xxx]], contents = function(m) filler_region(2,2); } }); end, },  { name = 'T-shaped, rot 3', contents = function() des.map({ map = [[ xxx----- xxx|...| xxx|...| ----...| |......| |......| |......| ----...| xxx|...| xxx|...| xxx-----]], contents = function(m) filler_region(5,5); } }); end, },  { name = 'S-shaped', contents = function() des.map({ map = [[ -----xxx |...|xxx |...|xxx |...---- |......| |......| |......| ----...| xxx|...| xxx|...| xxx-----]], contents = function(m) filler_region(2,2); } }); end, },  { name = 'S-shaped, rot 1', contents = function() des.map({ map = [[ xxx-------- xxx|......| xxx|......| ----......| |......---- |......|xxx |......|xxx --------xxx]], contents = function(m) filler_region(5,5); } }); end, },  { name = 'Z-shaped', contents = function() des.map({ map = [[ xxx----- xxx|...| xxx|...| ----...| |......| |......| |......| |...---- |...|xxx |...|xxx -----xxx]], contents = function(m) filler_region(5,5); } }); end, },  { name = 'Z-shaped, rot 1', contents = function() des.map({ map = [[ --------xxx |......|xxx |......|xxx |......---- ----......| xxx|......| xxx|......| xxx--------]], contents = function(m) filler_region(2,2); } }); end, },  { name = 'Cross', contents = function() des.map({ map = [[ xxx-----xxx xxx|...|xxx xxx|...|xxx ----...---- |.........| |.........| |.........| ----...---- xxx|...|xxx xxx|...|xxx xxx-----xxx]], contents = function(m) filler_region(6,6); } }); end, },  { name = 'Four-leaf clover', contents = function() des.map({ map = [[ -----x----- |...|x|...| |...---...| |.........| ---.....--- xx|.....|xx ---.....--- |.........| |...---...| |...|x|...| -----x-----]], contents = function(m) filler_region(6,6); } }); end, },  { name = 'Water-surrounded vault', contents = function() des.map({ map = [[ }}}}}}
    } // --}
    }| + |}
    }| + |}
    } // --}
    }}}}}}]], contents = function(m)
    des.region({ region: [3,3,3,3], type: "themed", irregular: true, filled: 0, joined: false });
    ...
    const escape_items = ["scroll of teleportation", "ring of teleportation", "wand of teleportation", "wand of digging"];
                const itm = obj.new(escape_items[Math.random(escape_items.length)]);
                const itmcls = itm.class()
                const box
                if (itmcls[ "material" ] === "glass") {
                      // explicitly force chest to be unlocked
    const box = des.object({ id: "chest", coord: chest_spots[1], olocked: "no" });
                } else {
                      // accept random locked/unlocked state
                      const box = des.object({ id: "chest", coord: chest_spots[1] });
                };
                box.addcontent(itm)

                for (let i = 2; i <= #chest_spots; i++) {
    ...
    function lookup_by_name(name, checkfills) {
       ...
       if (checkfills) {
          for (let i = 1; i <= #themeroom_fills; i++) {
             if (themeroom_fills[i].name === name) {
                return i;
             }
          }
       } else {
          for (let i = 1; i <= #themerooms; i++) {
    ...
    function themerooms_generate() {
       if (debug_rm_idx !== null) {
          ...
          const actualrm = lookup_by_name("default", false);
          if (percent(50)) {
    ...
    function pre_themerooms_generate() {
       ...
       if (debug_themerm !== null && debug_rm_idx === null) {
          ...
          pline("Warning: themeroom '" + debug_themerm
                 + "' ! found in themerooms" + xtrainfo, true)
       }
       if (debug_fill !== null && debug_fill_idx === null) {
          ...
          pline("Warning: themeroom fill '" + debug_fill
                 + "' ! found in themeroom_fills" + xtrainfo, true)
       }
    }
       ...
       if (debug_fill_idx !== null) {
          if (is_eligible(themeroom_fills[debug_fill_idx], rm)) {
             ...
             pline("Warning: fill '" + themeroom_fills[debug_fill_idx].name
                    + "' is ! eligible in room that generated it")
          }
          return
       }
       const pick = null;
