...
function u_calc_moveamt(player) {
    ...
    if (player.usteed && player.umoved) {
        // C ref: allmain.c u_calc_moveamt() — mounted heroes use steed speed.
        moveamt = mcalcmove(player.usteed, true);
    } else {
        // C ref: allmain.c u_calc_moveamt() — use current form move rate.
        moveamt = Number(player?.type?.mmove || player?.speed || NORMAL_SPEED);
    }
    if (player.veryFast) {
