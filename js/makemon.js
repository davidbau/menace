...
import { mon_learns_traps, emits_light, set_mon_data, monsndx,
         ...
         is_ndemon, is_mercenary, M_AP_TYPE, mhe, DEADMONSTER, is_placeholder,
         little_to_big, is_mplayer } from './mondata.js';
...
function makemonGoodpos(map, x, y, ptr, mmflags = NO_MM_FLAGS, avoidMonpos = true) {
    ...
    if (ptr) {
        // C ref: teleport.c goodpos() — m_in_air checks flyer OR floater
        // (clingers excluded because fakemon.mundetected is 0).
        const isFlyer = !!((ptr.mflags1 || 0) & M1_FLY);
        const isFloater = (ptr.mlet === S_EYE || ptr.mlet === S_LIGHT);
        const m_in_air = isFlyer || isFloater;
        if (IS_POOL(loc.typ) && !ignoreWater) {
            // C ref: is_swimmer(mdat) || m_in_air(mtmp)
            const isSwimmer = !!((ptr.mflags1 || 0) & (M1_SWIM | M1_AMPHIBIOUS));
            if (!(isSwimmer || m_in_air)) return false;
        } else if (ptr.mlet === S_EEL && rn2(13) && !ignoreWater) {
            // C ref: goodpos() eel check — rn2(13) BEFORE ignorewater
            return false;
        } else if (IS_LAVA(loc.typ) && !ignoreLava) {
            // C ref: floating eye avoids lava, others need m_in_air or likes_lava
            if (ptr.mndx === PM_FLOATING_EYE) return false;
            const likesLava = (ptr.mndx === PM_FIRE_ELEMENTAL || ptr.mndx === PM_SALAMANDER);
            if (!(m_in_air || likesLava)) return false;
        }
...
function makemon_rnd_goodpos(map, ptr, mmflags = NO_MM_FLAGS) {
    // COLNO, ROWNO imported from const.js
    let lastNx = 2;
    let lastNy = 0;
